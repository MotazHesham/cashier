    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.orders.update", [$order->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('paid_up') ? 'is-invalid' : '' }}" type="number" name="paid_up" id="paid_up" value="{{ old('paid_up', $order->paid_up) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('voucher_code') ? 'is-invalid' : '' }}" name="voucher_code_id" id="voucher_code_id">
                    @foreach($voucher_codes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('voucher_code_id') ? old('voucher_code_id') : $order->voucher_code->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
