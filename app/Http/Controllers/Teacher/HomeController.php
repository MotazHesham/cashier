...
class HomeController extends Controller
{
    ...
    {
        // $products = OrderProduct::where('order_id',3)
        //                           ->groupBy('product_id','attributes')
        //                           ->select('attributes')
        //                           ->selectRaw('sum(total_cost) as total_cost,sum(quantity) as quantity, product_id')
        //                           ->get();
        //return $products;
        $grade = $request->grade ? $request->grade : 1;
        $class = $request->class ? $request->class : 'a';
        $user = Auth::user();
